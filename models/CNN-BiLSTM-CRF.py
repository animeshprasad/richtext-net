...
#     model = TimeDistributed(Dense(hidden_dim//2, activation='relu'))(model)
#     model = Dropout(drop)(model)
#     out = TimeDistributed(Dense(1, activation='sigmoid'))(model)
    ...
    history = model.fit([X_train, X_train_char], Y_train_2, batch_size=64, epochs=epochs, validation_data=([X_val, X_val_char], Y_val_2)) 


    preds = model.predict([X_val, X_val_char])
...
if __name__=='__main__':
	...
	# one epoch
	threshold = 0.5
	model, history, p, r, f = run(X_train, X_train_char, Y_train, X_val, X_val_char, Y_val, embedding_matrix, vocab_size, max_sent_len=40, max_word_len=10, word_emb_dim=300, char_emb_dim=300, num_filters=400, kernel_size=6, neg_ratio=0.015, hidden_dim=100, drop=0.3, r_drop=0, epochs=10)
