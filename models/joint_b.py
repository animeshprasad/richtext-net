...
from keras.layers import LSTM, Embedding, Dense, TimeDistributed, Dropout, Bidirectional, Input, Flatten, Permute, multiply
...
def run(X_train, Y_train_seq, Y_train_cls, X_val, Y_val_seq, Y_val_cls, embedding_matrix, vocab_size, maxlen=40, emb_dim=300, neg_ratio=0.0125, hidden_dim=100, drop=0.2, r_drop=0.1, epochs=10, kernel=256, window=6):
    ## attention block
...
#             a = RepeatVector(input_dim)(a)
    ...
    attention_mul =  attention_3d_block(lstm_seq)
#     attention_dense = TimeDistributed(Dense(hidden_dim, activation='relu'))(attention_mul)
    crf = CRF(2)
    mention = crf(attention_mul)
    
    ## TO TRY: 1D Conv here instead of LSTM
#     dataset = Bidirectional(LSTM(hidden_dim, recurrent_dropout=r_drop))(attention_mul)
#     dataset = Dropout(drop)(dataset)
#     dataset_sum = Lambda(lambda x: keras.backend.sum(x, axis=1))(attention_mul)
    state_h = layers.Concatenate(axis=-1)([state_h_fw, state_h_bw])
    
#     dataset = Conv1D(kernel, window, activation='relu')(lstm_seq)
#     dataset = GlobalMaxPooling1D()(dataset)
    
#     state_cat = layers.Concatenate(axis=-1)([state_h, dataset_sum, dataset])
#     state_cat = layers.Concatenate(axis=-1)([state_h, dataset_sum])
    state_cat = Dropout(drop)(state_h)
    
    data_id = Dense(DATASET_CLASS, activation='sigmoid')(state_cat)
...
if not os.path.exists('../../data/val_segs/'):
        os.makedirs('../../data/val_segs/')
