...
def run(X_train, Y_train, X_val, Y_val, embedding_matrix, vocab_size, maxlen=40, emb_dim=300, kernel=64, window=5):
    ##build model
    model = Sequential()
    model.add(layers.Embedding(vocab_size, emb_dim, weights=[embedding_matrix], input_length=maxlen, trainable=False))
    model.add(layers.Conv1D(kernel, window, activation='relu'))
...
if __name__=='__main__':
	...
	X_train, Y_train, X_val, Y_val, embedding_matrix, vocab_size, tokenizer = prep_data(neg_ratio=0.025)
	model, history, p, r, f  = run(X_train, Y_train, X_val, Y_val, embedding_matrix, vocab_size, kernel=128, window=5)
